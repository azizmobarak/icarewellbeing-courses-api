            ...
            if (result) {
                ...
                await checkModuleName(req.body.module, id)
                    .then(async (moduleId) => {
                        ...
                        await getAuthor(id).then(async author=>{
                            if (!author){
                              return  createResponse(403,'not a User', res);
                            }else {
                              await countVideosByModule(module_id).then(videoNumber=>{
                                  if(videoNumber !== -1){
                                      const data = {
                                          user_id: id,
...
const getAuthor = async (user_id: string): Promise<null| string> =>{
  ...
  return await users.collection.findOne({_id: sanitize(new ObjectId(user_id))}).then((doc: any)=>{
      if(doc){
          return doc.username;
      }
      return null;
...
const countVideosByModule = async(module_id: string): Promise<number> => {
 ...
 return await course.collection.countDocuments({module: sanitize(module_id)}).then(count=>{
     return count + 1;
 }).catch(err=>{
     console.log(err);
     return -1;
